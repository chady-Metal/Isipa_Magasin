    ...
    <div class="min-h-screen">
        <header class="sticky top-0 z-30 border-b border-white/30 bg-white/75 backdrop-blur-xl">
            ...
            <div class="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
                <a href="{{ route('home') }}" class="flex items-center gap-2" data-async-link>
                ...
                <nav class="hidden items-center gap-2 md:flex">
                    <a href="{{ route('home') }}" data-async-link class="store-nav-link {{ request()->routeIs('home') ? 'store-nav-link-active' : '' }}">Accueil</a>
                    <a href="{{ route('catalogue.index') }}" data-async-link class="store-nav-link {{ request()->routeIs('catalogue.*') ? 'store-nav-link-active' : '' }}">Catalogue</a>

                    @auth
                        @if (auth()->user()->isClient())
                            <a href="{{ route('client.panier.index') }}" data-async-link class="store-nav-link {{ request()->routeIs('client.panier.*') ? 'store-nav-link-active' : '' }}">Panier</a>
                            <a href="{{ route('client.favoris.index') }}" data-async-link class="store-nav-link {{ request()->routeIs('client.favoris.*') ? 'store-nav-link-active' : '' }}">Favoris</a>
                            <a href="{{ route('client.commandes.index') }}" data-async-link class="store-nav-link {{ request()->routeIs('client.commandes.*') ? 'store-nav-link-active' : '' }}">Commandes</a>
                            <a href="{{ route('client.reclamations.index') }}" data-async-link class="store-nav-link {{ request()->routeIs('client.reclamations.*') ? 'store-nav-link-active' : '' }}">Reclamations</a>
                        @endif

                        @if (auth()->user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="store-nav-link">Administration</a>
                        @endif
                    @endauth
                </nav>
                ...
                <div class="flex items-center gap-2">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="store-btn-secondary">Connexion client</a>
        ...
        <main class="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8" data-page-content>
            @if (session('success'))
