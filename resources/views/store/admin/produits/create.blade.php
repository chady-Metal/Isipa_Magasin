...
@section('content')
    <section class="mb-6 flex items-end justify-between">
        <div>
            ...
            <h1 class="section-title mt-3">Ajouter un Produit</h1>
        </div>
        <a href="{{ route('admin.produits.index') }}" class="store-btn-secondary" data-async-link>Retour</a>
    </section>

    <section class="admin-card max-w-5xl">
        <form method="POST" action="{{ route('admin.produits.store') }}" class="grid gap-4 sm:grid-cols-2" data-async>
            @csrf
            @include('store.admin.produits.partials.form-fields')
